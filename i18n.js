/* i18n.js - 双语切换字典 */
var dict = {
  "index.0": {
    "zh": "奇幻假期 FANTASTIC VACATION | 全球高端游艇定制与私人海享生活缔造者",
    "en": "Fantastic Vacation | Global Luxury Yacht Customization & Private Marine Lifestyle Creator"
  },
  "index.1": {
    "zh": "FANTASTIC VACATION",
    "en": "FANTASTIC VACATION"
  },
  "index.2": {
    "zh": "LOADING",
    "en": "LOADING"
  },
  "index.3": {
    "zh": "FANTASTIC VACATION",
    "en": "FANTASTIC VACATION"
  },
  "index.4": {
    "zh": "EN",
    "en": "中文"
  },
  "index.5": {
    "zh": "→ 探索全系游艇",
    "en": "→ Explore All Yachts"
  },
  "index.6": {
    "zh": "→ 定制专属游艇",
    "en": "→ Custom Yacht"
  },
  "index.7": {
    "zh": "→ 浏览全球航线",
    "en": "→ Browse Global Routes"
  },
  "index.8": {
    "zh": "→ 联系专业顾问",
    "en": "→ Contact Advisor"
  },
  "index.9": {
    "zh": "奇幻假期",
    "en": "Fantastic Vacation"
  },
  "index.10": {
    "zh": "首页",
    "en": "Home"
  },
  "index.11": {
    "zh": "关于我们",
    "en": "About Us"
  },
  "index.12": {
    "zh": "公司简介",
    "en": "Company Profile"
  },
  "index.13": {
    "zh": "发展历程",
    "en": "History"
  },
  "index.14": {
    "zh": "企业文化",
    "en": "Corporate Culture"
  },
  "index.15": {
    "zh": "荣誉资质",
    "en": "Honors & Certifications"
  },
  "index.16": {
    "zh": "业务板块",
    "en": "Business"
  },
  "index.17": {
    "zh": "全案定制",
    "en": "Full Customization"
  },
  "index.18": {
    "zh": "租赁航线",
    "en": "Charter Routes"
  },
  "index.19": {
    "zh": "托管维保",
    "en": "Management & Maintenance"
  },
  "index.20": {
    "zh": "全球合作",
    "en": "Global Partnership"
  },
  "index.21": {
    "zh": "产品与服务",
    "en": "Products & Services"
  },
  "index.22": {
    "zh": "全系游艇",
    "en": "All Yachts"
  },
  "index.23": {
    "zh": "客户案例",
    "en": "Customer Cases"
  },
  "index.24": {
    "zh": "新闻中心",
    "en": "News Center"
  },
  "index.25": {
    "zh": "公司动态",
    "en": "Company News"
  },
  "index.26": {
    "zh": "行业资讯",
    "en": "Industry News"
  },
  "index.27": {
    "zh": "媒体报道",
    "en": "Media Reports"
  },
  "index.28": {
    "zh": "公告通知",
    "en": "Announcements"
  },
  "index.29": {
    "zh": "投资者关系",
    "en": "Investor Relations"
  },
  "index.30": {
    "zh": "联系我们",
    "en": "Contact Us"
  },
  "index.31": {
    "zh": "电话：13797920792 / +86 0755 3353-0188",
    "en": "Tel: 13797920792 / +86 0755 3353-01888"
  },
  "index.32": {
    "zh": "邮箱：hs@fantastic-vacation.com",
    "en": "Email: hs@fantastic-vacation.com"
  },
  "index.33": {
    "zh": "✦ EST. 2010 · GLOBAL LUXURY MARINE ✦",
    "en": "✦ EST. 2010 · GLOBAL LUXURY MARINE ✦"
  },
  "index.34": {
    "zh": "定义海洋奢华",
    "en": "Define Marine Luxury"
  },
  "index.35": {
    "zh": "从概念设计到远洋交付，从日间巡航到环球航行，我们重新定义每一寸海洋体验。",
    "en": "From concept design to ocean delivery, from day cruising to global voyages, we redefine every inch of the marine experience."
  },
  "index.36": {
    "zh": "探索全系游艇",
    "en": "Explore All Yachts"
  },
  "index.37": {
    "zh": "预约专属顾问",
    "en": "Book Exclusive Advisor"
  },
  "index.38": {
    "zh": "SCROLL",
    "en": "SCROLL"
  },
  "index.39": {
    "zh": "年行业深耕",
    "en": "Years of Industry Expertise"
  },
  "index.40": {
    "zh": "全球高净值客户",
    "en": "Global High-Net-Worth Clients"
  },
  "index.41": {
    "zh": "运营豪华游艇",
    "en": "Luxury Yachts Operated"
  },
  "index.42": {
    "zh": "覆盖国际海域",
    "en": "Covering International Waters"
  },
  "index.43": {
    "zh": "为何选择奇幻假期",
    "en": "Why Choose FANTASTIC VACATION"
  },
  "index.44": {
    "zh": "为何选择奇幻假期",
    "en": "Why Choose FANTASTIC VACATION"
  },
  "index.45": {
    "zh": "我们不只是游艇服务商，更是您海洋生活方式的全方位策展人",
    "en": "We are not just a yacht service provider, but your all-around curator of marine lifestyle"
  },
  "index.46": {
    "zh": "⚓",
    "en": "⚓"
  },
  "index.47": {
    "zh": "全产业链整合",
    "en": "Full Industry Chain Integration"
  },
  "index.48": {
    "zh": "从游艇研发设计、船厂制造监造，到交付后的运营管理、维修保养、保险合规——我们提供游艇全生命周期的一站式解决方案。",
    "en": "From yacht R&D design, shipyard manufacturing supervision, to post-delivery operation management, maintenance, insurance & compliance — we provide one-stop solutions for the entire yacht lifecycle."
  },
  "index.49": {
    "zh": "了解更多 →",
    "en": "Learn More →"
  },
  "index.50": {
    "zh": "🎨",
    "en": "🎨"
  },
  "index.51": {
    "zh": "顶级设计能力",
    "en": "Top-Tier Design Capability"
  },
  "index.52": {
    "zh": "携手意大利、荷兰、德国等全球顶尖船舶设计事务所，将艺术美学与工程性能完美融合，每艘游艇都是独一无二的海上艺术品。",
    "en": "Partnered with world's top naval architecture firms in Italy, Netherlands, Germany, perfectly blending artistic aesthetics with engineering performance — each yacht is a unique seafaring artwork."
  },
  "index.53": {
    "zh": "探索定制 →",
    "en": "Explore Custom →"
  },
  "index.54": {
    "zh": "🌍",
    "en": "🌍"
  },
  "index.55": {
    "zh": "全球网络覆盖",
    "en": "Global Network Coverage"
  },
  "index.56": {
    "zh": "在地中海、加勒比海、东南亚、中东、北欧等全球30+热门海域设有服务基地，无论您的航程指向何方，我们都能即时响应。",
    "en": "Service bases in 30+ popular waters worldwide including Mediterranean, Caribbean, Southeast Asia, Middle East, Northern Europe — wherever your voyage leads, we respond instantly."
  },
  "index.57": {
    "zh": "查看航线 →",
    "en": "View Routes →"
  },
  "index.58": {
    "zh": "🛡️",
    "en": "🛡️"
  },
  "index.59": {
    "zh": "管家式服务体系",
    "en": "Butler-Style Service System"
  },
  "index.60": {
    "zh": "每位客户配备专属Yacht Manager，7×24小时响应需求，从航线规划到岸上行程，真正实现\"拎包出海\"的无忧体验。",
    "en": "Each client is assigned a dedicated Yacht Manager, 7×24h response, from route planning to onshore itineraries — truly realizing a worry-free \"pack & sail\" experience."
  },
  "index.61": {
    "zh": "了解托管 →",
    "en": "Learn About Management →"
  },
  "index.62": {
    "zh": "四大核心系列",
    "en": "Four Core Series"
  },
  "index.63": {
    "zh": "四大核心系列",
    "en": "Four Core Series"
  },
  "index.64": {
    "zh": "覆盖从精致日间巡航到百米超级游艇的全谱系产品矩阵",
    "en": "Covering the full product matrix from refined day cruising to 100m superyachts"
  },
  "index.65": {
    "zh": "旗舰系列",
    "en": "Flagship Series"
  },
  "index.66": {
    "zh": "60-120m",
    "en": "60-120m"
  },
  "index.67": {
    "zh": "总长度",
    "en": "Length Overall"
  },
  "index.68": {
    "zh": "最大载客",
    "en": "Max Guests"
  },
  "index.69": {
    "zh": "无限航区",
    "en": "Unlimited Navigation"
  },
  "index.70": {
    "zh": "航行等级",
    "en": "Navigation Class"
  },
  "index.71": {
    "zh": "专为全球顶级富豪打造，融合宫殿级居住空间与尖端航海科技。",
    "en": "Built for the world's top billionaires, blending palace-grade living space with cutting-edge maritime technology."
  },
  "index.72": {
    "zh": "查看全系 →",
    "en": "View Full Series →"
  },
  "index.73": {
    "zh": "远征系列",
    "en": "Expedition Series"
  },
  "index.74": {
    "zh": "40-70m",
    "en": "40-70m"
  },
  "index.75": {
    "zh": "总长度",
    "en": "Length Overall"
  },
  "index.76": {
    "zh": "最大载客",
    "en": "Max Guests"
  },
  "index.77": {
    "zh": "极地/冰级",
    "en": "Polar/Ice Class"
  },
  "index.78": {
    "zh": "特殊认证",
    "en": "Special Certification"
  },
  "index.79": {
    "zh": "探险型远洋游艇，可深入南极、阿拉斯加等极地海域，不失五星舒适度。",
    "en": "Expedition superyacht, capable of reaching polar waters like Antarctica and Alaska without compromising 5-star comfort."
  },
  "index.80": {
    "zh": "查看全系 →",
    "en": "View Full Series →"
  },
  "index.81": {
    "zh": "飞桥系列",
    "en": "Flybridge Series"
  },
  "index.82": {
    "zh": "18-45m",
    "en": "18-45m"
  },
  "index.83": {
    "zh": "总长度",
    "en": "Length Overall"
  },
  "index.84": {
    "zh": "最大载客",
    "en": "Max Guests"
  },
  "index.85": {
    "zh": "28kn+",
    "en": "28kn+"
  },
  "index.86": {
    "zh": "最高航速",
    "en": "Max Speed"
  },
  "index.87": {
    "zh": "经典飞桥运动游艇，适合地中海沿岸巡航与岛屿跳岛之旅。",
    "en": "Classic flybridge motoryacht, ideal for Mediterranean coastal cruising and island hopping."
  },
  "index.88": {
    "zh": "查看全系 →",
    "en": "View Full Series →"
  },
  "index.89": {
    "zh": "日间系列",
    "en": "Day Cruiser Series"
  },
  "index.90": {
    "zh": "10-18m",
    "en": "10-18m"
  },
  "index.91": {
    "zh": "总长度",
    "en": "Length Overall"
  },
  "index.92": {
    "zh": "最大载客",
    "en": "Max Guests"
  },
  "index.93": {
    "zh": "35kn+",
    "en": "35kn+"
  },
  "index.94": {
    "zh": "最高航速",
    "en": "Max Speed"
  },
  "index.95": {
    "zh": "精致紧凑的日间巡航游艇，入门级奢华海享方案。",
    "en": "Refined and compact day cruiser, an entry-level luxury marine solution."
  },
  "index.96": {
    "zh": "查看全系 →",
    "en": "View Full Series →"
  },
  "index.97": {
    "zh": "浏览全部50艘游艇 →",
    "en": "Browse All 50 Yachts →"
  },
  "index.98": {
    "zh": "浏览全部50艘游艇 →",
    "en": "Browse All 50 Yachts →"
  },
  "index.99": {
    "zh": "\"我们不制造游艇，我们编织海洋梦想。\"",
    "en": "\"We don't build yachts, we weave ocean dreams.\""
  },
  "index.100": {
    "zh": "\"我们不制造游艇，我们编织海洋梦想。\"",
    "en": "\"We don't build yachts, we weave ocean dreams.\""
  },
  "index.101": {
    "zh": "全流程定制能力",
    "en": "Full-Process Customization Capability"
  },
  "index.102": {
    "zh": "全流程定制能力",
    "en": "Full-Process Customization Capability"
  },
  "index.103": {
    "zh": "从最初的概念草图到最终的海上交付，奇幻假期的定制服务涵盖游艇诞生的每一个环节。",
    "en": "From initial concept sketches to final ocean delivery, Fantastic Vacation's customization service covers every stage of a yacht's birth."
  },
  "index.104": {
    "zh": "我们的国际化团队汇聚了来自意大利、荷兰、德国、日本的顶尖设计师与工程师，在船体流体力学、内部空间规划、智能系统集成、环保动力方案等领域拥有深厚的专业积累。",
    "en": "Our international team brings together top designers and engineers from Italy, Netherlands, Germany, and Japan, with deep expertise in hull hydrodynamics, interior space planning, smart system integration, and eco-friendly propulsion solutions."
  },
  "index.105": {
    "zh": "外观设计",
    "en": "Exterior Design"
  },
  "index.106": {
    "zh": "联合全球顶级船舶设计工作室，打造独一无二的视觉语言",
    "en": "In partnership with world's top naval design studios to create a unique visual language"
  },
  "index.107": {
    "zh": "内饰配置",
    "en": "Interior Configuration"
  },
  "index.108": {
    "zh": "精选意大利皮革、法国橡木、西班牙大理石等顶级材料",
    "en": "Premium materials selected: Italian leather, French oak, Spanish marble"
  },
  "index.109": {
    "zh": "功能系统",
    "en": "Function Systems"
  },
  "index.110": {
    "zh": "稳定系统、减摇鳍、零速鳍、岸电系统全面定制",
    "en": "Stabilization systems, anti-roll fins, zero-speed fins, shore power — fully customized"
  },
  "index.111": {
    "zh": "智能科技",
    "en": "Smart Technology"
  },
  "index.112": {
    "zh": "全船智能中控、卫星通信、水下灯光、影院系统",
    "en": "Full-vessel smart control, satellite comms, underwater lighting, cinema systems"
  },
  "index.113": {
    "zh": "开启定制之旅",
    "en": "Start Your Custom Journey"
  },
  "index.114": {
    "zh": "精选全球航线",
    "en": "Selected Global Routes"
  },
  "index.115": {
    "zh": "精选全球航线",
    "en": "Selected Global Routes"
  },
  "index.116": {
    "zh": "从地中海的蔚蓝海岸到加勒比的珍珠群岛，为您呈现地球上最令人心动的海上旅程",
    "en": "From the azure coast of the Mediterranean to the Pearl Islands of the Caribbean, presenting the most breathtaking sea journeys on Earth"
  },
  "index.117": {
    "zh": "地中海",
    "en": "Mediterranean"
  },
  "index.118": {
    "zh": "7-14天",
    "en": "7-14 Days"
  },
  "index.119": {
    "zh": "蔚蓝海岸 · 地中海经典环线",
    "en": "Azure Coast · Classic Mediterranean Circuit"
  },
  "index.120": {
    "zh": "摩纳哥→尼斯→戛纳→热那亚→阿马尔菲海岸，途经世界顶级游艇俱乐部。",
    "en": "Monaco → Nice → Cannes → Genoa → Amalfi Coast, passing the world's top yacht clubs."
  },
  "index.121": {
    "zh": "€85,000起/周",
    "en": "From €85,000/week"
  },
  "index.122": {
    "zh": "含船长船员",
    "en": "Incl. Captain & Crew"
  },
  "index.123": {
    "zh": "加勒比海",
    "en": "Caribbean"
  },
  "index.124": {
    "zh": "10-21天",
    "en": "10-21 Days"
  },
  "index.125": {
    "zh": "珍珠群岛 · 加勒比海奢享之旅",
    "en": "Pearl Islands · Caribbean Luxury Journey"
  },
  "index.126": {
    "zh": "巴哈马→特克斯凯科斯→英属维尔京群岛，清澈温水与隐秘海湾。",
    "en": "Bahamas → Turks & Caicos → British Virgin Islands, crystal-clear warm waters and hidden bays."
  },
  "index.127": {
    "zh": "$95,000起/周",
    "en": "From $95,000/week"
  },
  "index.128": {
    "zh": "含全套服务",
    "en": "Full Service Included"
  },
  "index.129": {
    "zh": "东南亚",
    "en": "Southeast Asia"
  },
  "index.130": {
    "zh": "5-14天",
    "en": "5-14 Days"
  },
  "index.131": {
    "zh": "翡翠海域 · 安达曼海秘境巡游",
    "en": "Jade Waters · Andaman Sea Secret Cruise"
  },
  "index.132": {
    "zh": "普吉岛→皮皮岛→缅甸丹老群岛，丰富水上活动与地道泰式料理。",
    "en": "Phuket → Phi Phi → Myanmar's Mergui Archipelago, rich water activities & authentic Thai cuisine."
  },
  "index.133": {
    "zh": "¥380,000起/周",
    "en": "From ¥380,000/week"
  },
  "index.134": {
    "zh": "含全套服务",
    "en": "Full Service Included"
  },
  "index.135": {
    "zh": "查看全部航线 →",
    "en": "View All Routes →"
  },
  "index.136": {
    "zh": "查看全部航线 →",
    "en": "View All Routes →"
  },
  "index.137": {
    "zh": "360° 托管维保体系",
    "en": "360° Management & Maintenance System"
  },
  "index.138": {
    "zh": "360° 托管维保体系",
    "en": "360° Management & Maintenance System"
  },
  "index.139": {
    "zh": "专业的游艇资产管理服务，让您的海上资产始终处于最佳状态",
    "en": "Professional yacht asset management services, keeping your marine assets in peak condition at all times"
  },
  "index.140": {
    "zh": "📋",
    "en": "📋"
  },
  "index.141": {
    "zh": "日常运营",
    "en": "Daily Operations"
  },
  "index.142": {
    "zh": "船员管理、日程调度、财务核算",
    "en": "Crew management, schedule coordination, financial accounting"
  },
  "index.143": {
    "zh": "🔧",
    "en": "🔧"
  },
  "index.144": {
    "zh": "定期保养",
    "en": "Regular Maintenance"
  },
  "index.145": {
    "zh": "发动机保养、防污漆更新、设备检测",
    "en": "Engine maintenance, antifouling paint renewal, equipment inspection"
  },
  "index.146": {
    "zh": "🔩",
    "en": "🔩"
  },
  "index.147": {
    "zh": "专业维修",
    "en": "Professional Repair"
  },
  "index.148": {
    "zh": "紧急抢修、船体修复、系统升级",
    "en": "Emergency repair, hull restoration, system upgrades"
  },
  "index.149": {
    "zh": "📜",
    "en": "📜"
  },
  "index.150": {
    "zh": "合规年检",
    "en": "Compliance Annual Inspection"
  },
  "index.151": {
    "zh": "船级社检验、法定证书更新",
    "en": "Classification society inspection, statutory certificate renewal"
  },
  "index.152": {
    "zh": "🛡️",
    "en": "🛡️"
  },
  "index.153": {
    "zh": "保险理赔",
    "en": "Insurance Claims"
  },
  "index.154": {
    "zh": "H&M安排、P&I对接、事故处理",
    "en": "H&M arrangement, P&I coordination, incident handling"
  },
  "index.155": {
    "zh": "了解完整托管方案 →",
    "en": "Learn Full Management Plan →"
  },
  "index.156": {
    "zh": "了解完整托管方案 →",
    "en": "Learn Full Management Plan →"
  },
  "index.157": {
    "zh": "精选项目案例",
    "en": "Selected Project Cases"
  },
  "index.158": {
    "zh": "精选项目案例",
    "en": "Selected Project Cases"
  },
  "index.159": {
    "zh": "每一个案例都是我们对卓越的不懈追求",
    "en": "Every case is our relentless pursuit of excellence"
  },
  "index.160": {
    "zh": "全案定制",
    "en": "Full Customization"
  },
  "index.161": {
    "zh": "82米超级游艇「幻蓝号」",
    "en": "82m Superyacht \"Fantasy Blue\""
  },
  "index.162": {
    "zh": "中东王室委托，历时38个月，搭载直升机平台、潜艇车库、IMAX影院。2024年荣获Asia Boating Awards年度最佳定制游艇。",
    "en": "Commissioned by Middle East royal family, 38 months in build, featuring helipad, submarine garage, IMAX theater. Winner of Asia Boating Awards 2024 Custom Yacht of the Year."
  },
  "index.163": {
    "zh": "租赁航行",
    "en": "Charter Voyage"
  },
  "index.164": {
    "zh": "地中海30天深度巡航",
    "en": "30-Day Mediterranean Deep Cruise"
  },
  "index.165": {
    "zh": "欧洲家族企业定制，途经12个国家24个港口，配备米其林主厨与私人潜水教练。",
    "en": "Customized for European family business, through 24 ports in 12 countries, with Michelin-starred chef and private dive instructor."
  },
  "index.166": {
    "zh": "托管服务",
    "en": "Management Service"
  },
  "index.167": {
    "zh": "中东王室船队5年全托管",
    "en": "Middle East Royal Fleet 5-Year Full Management"
  },
  "index.168": {
    "zh": "管理3艘超级游艇（45m/62m/88m），5年零重大事故运营记录。",
    "en": "Managing 3 superyachts (45m/62m/88m), zero major incidents in 5-year operation record."
  },
  "index.169": {
    "zh": "查看全部案例 →",
    "en": "View All Cases →"
  },
  "index.170": {
    "zh": "查看全部案例 →",
    "en": "View All Cases →"
  },
  "index.171": {
    "zh": "新闻与动态",
    "en": "News & Updates"
  },
  "index.172": {
    "zh": "新闻与动态",
    "en": "News & Updates"
  },
  "index.173": {
    "zh": "了解奇幻假期的最新动态与海洋生活方式资讯",
    "en": "Stay updated with Fantastic Vacation's latest news and marine lifestyle insights"
  },
  "index.174": {
    "zh": "品牌动态",
    "en": "Brand News"
  },
  "index.175": {
    "zh": "奇幻假期香港运营中心正式启用",
    "en": "Fantastic Vacation Hong Kong Operation Center Officially Opened"
  },
  "index.176": {
    "zh": "2026年5月，奇幻假期在香港中环设立亚太区总部，统筹亚太地区业务。",
    "en": "May 2026, Fantastic Vacation establishes Asia-Pacific HQ in Central, Hong Kong, overseeing regional operations."
  },
  "index.177": {
    "zh": "行业趋势",
    "en": "Industry Trends"
  },
  "index.178": {
    "zh": "新能源动力游艇增长超40%",
    "en": "New Energy Yacht Orders Up 40%+"
  },
  "index.179": {
    "zh": "混合动力与纯电动游艇订单量大幅增长，环保合规成为船东核心考量。",
    "en": "Hybrid and full-electric yacht orders surged significantly, with environmental compliance becoming a core owner consideration."
  },
  "index.180": {
    "zh": "生活方式",
    "en": "Lifestyle"
  },
  "index.181": {
    "zh": "海上派对策划完整指南",
    "en": "Complete Guide to Yacht Party Planning"
  },
  "index.182": {
    "zh": "从主题布置到菜单设计，活动策划专家分享海上派对举办攻略。",
    "en": "From theme decoration to menu design, event planning experts share tips for hosting epic yacht parties."
  },
  "index.183": {
    "zh": "更多资讯 →",
    "en": "More News →"
  },
  "index.184": {
    "zh": "更多资讯 →",
    "en": "More News →"
  },
  "index.185": {
    "zh": "全球合作网络",
    "en": "Global Partnership Network"
  },
  "index.186": {
    "zh": "全球合作网络",
    "en": "Global Partnership Network"
  },
  "index.187": {
    "zh": "携手世界顶级合作伙伴，共建海洋奢华生态系统",
    "en": "Partnering with world's top partners to build a marine luxury ecosystem"
  },
  "index.188": {
    "zh": "🏗️ 顶级船厂合作",
    "en": "🏗️ Top Shipyard Partners"
  },
  "index.189": {
    "zh": "与Ferretti Group、Feadship、Lürssen等世界一流造船集团建立战略伙伴关系。",
    "en": "Strategic partnerships with world-class shipbuilding groups including Ferretti Group, Feadship, Lürssen."
  },
  "index.190": {
    "zh": "🏝️ Marina网络",
    "en": "🏝️ Marina Network"
  },
  "index.191": {
    "zh": "覆盖全球80+顶级游艇码头，为客户提供专属泊位预订与VIP接待服务。",
    "en": "Covering 80+ top global marinas, providing clients with exclusive berth booking and VIP reception services."
  },
  "index.192": {
    "zh": "✈️ 奢华生活配套",
    "en": "✈️ Luxury Lifestyle Support"
  },
  "index.193": {
    "zh": "整合私人航空、顶级酒店、艺术品收藏等资源，打造无缝奢华体验链。",
    "en": "Integrating private aviation, top hotels, art collection and more, creating a seamless luxury experience chain."
  },
  "index.194": {
    "zh": "了解合作详情 →",
    "en": "Learn Partnership Details →"
  },
  "index.195": {
    "zh": "了解合作详情 →",
    "en": "Learn Partnership Details →"
  },
  "index.196": {
    "zh": "海上传奇",
    "en": "Legendary Voyage"
  },
  "index.197": {
    "zh": "无论您正在寻找梦想中的第一艘游艇，还是希望升级现有服务体验，我们的专业顾问团队随时为您效劳。",
    "en": "Whether you are looking for your dream first yacht or wish to upgrade your current service experience, our professional advisory team is at your service."
  },
  "index.198": {
    "zh": "📞 +86 0755 3353-0188",
    "en": "📞 +86 0755 3353-01888"
  },
  "index.199": {
    "zh": "✉️ hs@fantastic-vacation.com",
    "en": "✉️ hs@fantastic-vacation.com"
  },
  "index.200": {
    "zh": "立即预约咨询",
    "en": "Book Consultation Now"
  },
  "index.201": {
    "zh": "奇幻假期",
    "en": "Fantastic Vacation"
  },
  "index.202": {
    "zh": "FANTASTIC VACATION",
    "en": "FANTASTIC VACATION"
  },
  "index.203": {
    "zh": "奇幻假期实业有限公司深耕高端游艇行业15年，为200+全球高净值客户提供游艇定制、租赁、托管全产业链一站式服务。",
    "en": "Fantastic Vacation Industrial Co., Ltd. has been deeply engaged in the high-end yacht industry for 15 years, providing 200+ global high-net-worth clients with one-stop services across yacht customization, charter, and management."
  },
  "index.204": {
    "zh": "微信公众号",
    "en": "WeChat Official Account"
  },
  "index.205": {
    "zh": "微博",
    "en": "Weibo"
  },
  "index.206": {
    "zh": "抖音",
    "en": "TikTok (Douyin)"
  },
  "index.207": {
    "zh": "LinkedIn",
    "en": "LinkedIn"
  },
  "index.208": {
    "zh": "YouTube",
    "en": "YouTube"
  },
  "index.209": {
    "zh": "首页",
    "en": "Home"
  },
  "index.210": {
    "zh": "首页",
    "en": "Home"
  },
  "index.211": {
    "zh": "关于我们",
    "en": "About Us"
  },
  "index.212": {
    "zh": "全系游艇",
    "en": "All Yachts"
  },
  "index.213": {
    "zh": "全案定制",
    "en": "Full Customization"
  },
  "index.214": {
    "zh": "租赁航线",
    "en": "Charter Routes"
  },
  "index.215": {
    "zh": "托管维保",
    "en": "Management & Maintenance"
  },
  "index.216": {
    "zh": "案例展示",
    "en": "Case Showcase"
  },
  "index.217": {
    "zh": "新闻资讯",
    "en": "News & Information"
  },
  "index.218": {
    "zh": "全球合作",
    "en": "Global Partnership"
  },
  "index.219": {
    "zh": "网站地图",
    "en": "Site Map"
  },
  "index.220": {
    "zh": "公司地址",
    "en": "Company Address"
  },
  "index.221": {
    "zh": "咨询热线",
    "en": "Consultation Hotline"
  },
  "index.222": {
    "zh": "传真",
    "en": "Fax"
  },
  "index.223": {
    "zh": "商务邮箱",
    "en": "Business Email"
  },
  "index.224": {
    "zh": "hs@fantastic-vacation.com",
    "en": "hs@fantastic-vacation.com"
  },
  "index.225": {
    "zh": "ISO 9001质量管理体系认证",
    "en": "ISO 9001 Quality Management System Certified"
  },
  "index.226": {
    "zh": "意大利Ferretti集团授权经销商",
    "en": "Authorized Dealer of Ferretti Group, Italy"
  },
  "index.227": {
    "zh": "荷兰Feadship技术合作伙伴",
    "en": "Technical Partner of Feadship, Netherlands"
  },
  "index.228": {
    "zh": "地中海船坞协会认证会员",
    "en": "Certified Member of Mediterranean Shipyard Association"
  },
  "index.229": {
    "zh": "亚洲最佳游艇服务商三连冠",
    "en": "Three-Time Winner: Asia's Best Yacht Service Provider"
  },
  "index.230": {
    "zh": "© 2020-2026 奇幻假期实业有限公司 FANTASTIC VACATION INDUSTRIAL CO., LTD. 保留所有权利。",
    "en": "© 2020-2026 Fantastic Vacation Industrial Co., Ltd. All Rights Reserved."
  },
  "index.231": {
    "zh": "隐私政策",
    "en": "Privacy Policy"
  },
  "index.232": {
    "zh": "使用条款",
    "en": "Terms of Use"
  },
  "index.233": {
    "zh": "网站地图",
    "en": "Site Map"
  }
};

/**
 * i18n 双语切换系统
 * 支持：中文(zh) / 英文(en)
 * 使用方法：给所有需要翻译的文字元素加上 data-i18n="key" 属性
 * 切换语言：i18n.switch('en') 或 i18n.switch('zh')
 */

const i18n = {
  currentLang: 'zh',

  // ========== 翻译词典 ==========
  dict: {
    // --- 导航栏 ---
    'nav.home':                       { zh: '首页',                             en: 'Home' },
    'nav.about':                      { zh: '关于我们',                         en: 'About Us' },
    'nav.about.profile':              { zh: '公司简介',                         en: 'Company Profile' },
    'nav.about.history':              { zh: '发展历程',                         en: 'History' },
    'nav.about.culture':              { zh: '企业文化',                         en: 'Corporate Culture' },
    'nav.about.structure':            { zh: '组织架构',                         en: 'Organization Structure' },
    'nav.about.honors':               { zh: '荣誉资质',                         en: 'Honors & Certifications' },
    'nav.about.responsibility':       { zh: '社会责任',                         en: 'Social Responsibility' },
    'nav.business':                   { zh: '业务板块',                         en: 'Business' },
    'nav.business.custom':            { zh: '全案定制',                         en: 'Full Customization' },
    'nav.business.charter':           { zh: '租赁航线',                         en: 'Charter Routes' },
    'nav.business.management':        { zh: '托管维保',                         en: 'Management & Maintenance' },
    'nav.business.partnership':       { zh: '全球合作',                         en: 'Global Partnership' },
    'nav.products':                   { zh: '产品与服务',                       en: 'Products & Services' },
    'nav.products.yachts':            { zh: '全系游艇',                         en: 'All Yachts' },
    'nav.products.solutions':         { zh: '定制解决方案',                     en: 'Custom Solutions' },
    'nav.products.charter_svc':       { zh: '租赁服务',                         en: 'Charter Services' },
    'nav.products.mgmt_svc':          { zh: '托管服务',                         en: 'Management Services' },
    'nav.products.cases':             { zh: '客户案例',                         en: 'Customer Cases' },
    'nav.news':                       { zh: '新闻中心',                         en: 'News Center' },
    'nav.news.company':               { zh: '公司动态',                         en: 'Company News' },
    'nav.news.industry':              { zh: '行业资讯',                         en: 'Industry News' },
    'nav.news.media':                 { zh: '媒体报道',                         en: 'Media Reports' },
    'nav.news.announce':              { zh: '公告通知',                         en: 'Announcements' },
    'nav.ir':                         { zh: '投资者关系',                       en: 'Investor Relations' },
    'nav.ir.profile':                 { zh: '公司概况',                         en: 'Company Overview' },
    'nav.ir.financial':               { zh: '财务报告',                         en: 'Financial Reports' },
    'nav.ir.announcements':           { zh: '公告披露',                         en: 'Disclosures' },
    'nav.contact':                    { zh: '联系我们',                         en: 'Contact Us' },
    'nav.search':                     { zh: '搜索',                             en: 'Search' },
    'nav.dark_mode':                  { zh: '深色模式',                         en: 'Dark Mode' },

    // --- 语言切换按钮 ---
    'lang.switch_to_en':              { zh: 'EN',                               en: '中文' },
    'lang.current_zh':                { zh: '中文',                             en: 'Chinese' },
    'lang.current_en':                { zh: 'EN',                               en: 'English' },

    // --- 页脚 ---
    'footer.copyright':               { zh: '© 2024 奇幻假期游艇集团 版权所有', en: '© 2024 Fantastic Vacation Yacht Group. All Rights Reserved.' },
    'footer.icp':                     { zh: '粤ICP备XXXXXXXX号',                en: 'Guangdong ICP No.XXXXXXXX' },
    'footer.nav.home':                { zh: '首页',                             en: 'Home' },
    'footer.nav.about':               { zh: '关于我们',                         en: 'About Us' },
    'footer.nav.business':            { zh: '业务板块',                         en: 'Business' },
    'footer.nav.products':            { zh: '产品与服务',                       en: 'Products & Services' },
    'footer.nav.news':                { zh: '新闻中心',                         en: 'News Center' },
    'footer.nav.ir':                  { zh: '投资者关系',                       en: 'Investor Relations' },
    'footer.nav.contact':             { zh: '联系我们',                         en: 'Contact Us' },
    'footer.contact.title':           { zh: '联系我们',                         en: 'Contact Us' },
    'footer.contact.phone':           { zh: '尊享热线',                         en: 'Exclusive Hotline' },
    'footer.contact.email':           { zh: '官方邮箱',                         en: 'Official Email' },
    'footer.contact.address':          { zh: '集团总部',                         en: 'Group Headquarters' },
    'footer.follow':                   { zh: '关注我们',                         en: 'Follow Us' },
    'footer.legal':                    { zh: '法律信息',                         en: 'Legal Information' },
    'footer.legal.privacy':            { zh: '隐私政策',                         en: 'Privacy Policy' },
    'footer.legal.terms':             { zh: '使用条款',                         en: 'Terms of Service' },
    'footer.legal.sitemap':           { zh: '网站地图',                         en: 'Sitemap' },

    // --- 搜索 ---
    'search.placeholder':              { zh: '搜索产品、新闻、案例...',           en: 'Search products, news, cases...' },
    'search.close':                   { zh: '关闭搜索',                         en: 'Close Search' },

    // --- 通用 ---
    'btn.more':                       { zh: '了解更多',                         en: 'Learn More' },
    'btn.view_all':                   { zh: '查看全部',                         en: 'View All' },
    'btn.submit':                     { zh: '提交',                             en: 'Submit' },
    'btn.contact_us':                 { zh: '联系我们',                         en: 'Contact Us' },
    'btn.download':                    { zh: '下载',                             en: 'Download' },
    'loading':                        { zh: '加载中...',                        en: 'Loading...' },
  },

  // ========== 核心方法 ==========

  /** 初始化：读取 localStorage，默认中文 */
  init() {
    const saved = localStorage.getItem('site-lang');
    if (saved === 'en' || saved === 'zh') {
      this.switch(saved, false);
    } else {
      this.switch('zh', false);
    }
  },

  /** 切换语言 */
  switch(lang, save = true) {
    this.currentLang = lang;
    if (save) localStorage.setItem('site-lang', lang);

    // 更新所有带 data-i18n 的元素
    document.querySelectorAll('[data-i18n]').forEach(el => {
      const key = el.getAttribute('data-i18n');
      const entry = this.dict[key];
      if (!entry) return;

      // 根据元素类型更新文字
      if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
        if (el.placeholder) el.placeholder = entry[lang];
      } else if (el.hasAttribute('data-i18n-html')) {
        // 保留内部 HTML 结构，只替换文字节点（高级用法）
        this._replaceTextNodes(el, entry[lang]);
      } else {
        el.textContent = entry[lang];
      }
    });

    // 更新 placeholder
    document.querySelectorAll('[data-i18n-placeholder]').forEach(el => {
      const key = el.getAttribute('data-i18n-placeholder');
      if (this.dict[key]) el.placeholder = this.dict[key][lang];
    });

    // 更新按钮文字
    const btn = document.getElementById('langSwitchBtn');
    if (btn) {
      btn.textContent = lang === 'zh' ? 'EN' : '中文';
    }

    // 更新 HTML lang 属性
    document.documentElement.lang = lang === 'zh' ? 'zh-CN' : 'en';

    // 触发自定义事件，供页面特定逻辑监听
    document.dispatchEvent(new CustomEvent('i18n:switched', { detail: { lang } }));
  },

  /** 替换文字节点（保留子标签） */
  _replaceTextNodes(el, newText) {
    // 简单处理：直接替换 textContent
    // 对于复杂 HTML 结构，需要在调用处用 data-i18n-html 手动处理
    el.textContent = newText;
  },

  /** 动态添加翻译词条（供各页面加载自己的词典） */
  addDict(newEntries) {
    Object.assign(this.dict, newEntries);
  }
};

// 页面加载后自动初始化
document.addEventListener('DOMContentLoaded', () => i18n.init());


/* === i18n 切换函数 === */
function applyI18n(lang) {
  if (typeof dict !== 'object') return;
  document.querySelectorAll('[data-i18n]').forEach(function(el) {
    var key = el.getAttribute('data-i18n');
    if (dict[key] && dict[key][lang]) {
      el.textContent = dict[key][lang];
    }
  });
  document.querySelectorAll('[data-i18n-attr]').forEach(function(el) {
    var spec = el.getAttribute('data-i18n-attr');
    if (!spec) return;
    spec.split(',').forEach(function(pair) {
      var parts = pair.trim().split(':');
      if (parts.length < 2) return;
      var attr = parts[0].trim();
      var key = parts[1].trim();
      if (dict[key] && dict[key][lang]) {
        el.setAttribute(attr, dict[key][lang]);
      }
    });
  });
}

function switchLang(lang) {
  if (lang !== 'zh' && lang !== 'en') lang = 'zh';
  try { localStorage.setItem('fv-lang', lang); } catch(e){}
  applyI18n(lang);
  var btn = document.getElementById('langSwitchBtn');
  if (btn) btn.textContent = lang === 'zh' ? 'EN' : '中文';
}

(function initI18n() {
  var saved = 'zh';
  try { saved = localStorage.getItem('fv-lang') || 'zh'; } catch(e){}
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() { switchLang(saved); });
  } else {
    switchLang(saved);
  }
  var btn = document.getElementById('langSwitchBtn');
  if (btn) {
    btn.addEventListener('click', function() {
      var cur = 'zh';
      try { cur = localStorage.getItem('fv-lang') || 'zh'; } catch(e){}
      switchLang(cur === 'zh' ? 'en' : 'zh');
    });
  }
})();
