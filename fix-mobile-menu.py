...
# 完整的移动端菜单和搜索框模板（从 index.html 提取）
...
print("🔧 修复缺失移动端菜单和搜索功能的页面...")

for page in pages_to_fix:
    file_path = BASE_DIR / page
    if not file_path.exists():
        print(f"  ⚠️  {page} 不存在，跳过")
        continue
    ...
    if 'id="mobile-menu"' in content:
        print(f"  ✅ {page} 已有移动端菜单，跳过")
        continue

    # 找到 </nav> 标签的位置，在其后插入移动端菜单和搜索框
    ...
    if nav_close_match:
        ...
        print(f"  ⚠️  {page} 找不到 </nav> 标签，无法插入")
